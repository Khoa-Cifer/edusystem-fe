...
export default function LessonsPage() {
  ...
  const templates = ["Standard Lesson", "Lab Activity", "Discussion", "Assessment"];
  ...
  useEffect(() => {
    ...
    const fetchLessons = async () => {
      try {
        ...
        console.error("Failed to fetch lessons", err?.response || err?.message || err);
      ...
      <div className="space-y-6">
        ...
        <Card className="p-4 bg-card border-border">
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <Button variant="outline" className="bg-transparent" aria-label="Filter lessons">
        ...
        {/* Lessons Grid */}
        {loading ? (
          <p className="text-center text-muted-foreground mt-6">Loading lessons...</p>
        ) : error ? (
          <p className="text-center text-red-500 mt-6">{error}</p>
        ) : filteredLessons.length === 0 ? (
          <p className="text-center text-muted-foreground mt-6">No lessons found.</p>
        ) : (
            ...
            {filteredLessons.map((lesson) => (
              ...
              >
                <div>
                  ...
                  {/* Lesson Info */}
                  <h3 className="font-semibold text-lg mb-2 line-clamp-2">{lesson.lessonName}</h3>
                  <div className="flex flex-wrap gap-2 mb-4">
                    <Badge variant="secondary" className="text-xs">{lesson.unitName}</Badge>
                    <Badge variant="outline" className="text-xs">{lesson.skill}</Badge>
                  </div>
                  <div className="flex items-center justify-between text-sm text-muted-foreground">
                    <span>{lesson.createTime ?? "N/A"}</span>
                    <span>{lesson.duration ? `${lesson.duration} min` : null}</span>
                  </div>
        ...
        <div className="mt-8">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
            {templates.map((template) => (
              ...
              >
                <Link
                  href={`/(teacher)/lessons/new?template=${encodeURIComponent(template)}`}
